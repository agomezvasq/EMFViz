...
public class TestParticleField : MonoBehaviour {
	...
	void Start () {
		...
		testParticles = new TestParticle [(int)(Screen.height / ssSize.y) + 1, (int)(Screen.width / ssSize.x) + 1, (int)(Screen.width / ssSize.x) + 1];
		...
		for (int i = 0; i < Rows (); i+=c) {
			for (int j = 0; j < Columns (); j+=c) {
				for (int k = 0; k < Aisles (); k+=c) {
					...
					position = new Vector3 (position.x, position.y, (k / c + 0.5f) - 2.0f * 9.5f);
