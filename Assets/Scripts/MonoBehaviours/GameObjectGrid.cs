...
public class GameObjectGrid : MonoBehaviour {
	...
	void Start () {
        ...
        int interval = Mathf.RoundToInt((float)(1D / resolution));
		for (int i = 0; i < Rows (); i += interval) {
			for (int j = 0; j < Columns (); j += interval) {
				for (int k = 0; k < Aisles (); k += interval) {
					Vector3 position = Camera.main.ScreenToWorldPoint (new Vector3 (ssSize.x * (j + 0.5f), ssSize.y * (i + 0.5f), 0.0f));
					position = new Vector3 (position.x, position.y, (k / interval + 0.5f) - 9.5f - 9.5f / 2.0f - 0.5f);
